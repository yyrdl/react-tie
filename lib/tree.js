 ...
 * @api public
 */
function dispatchDataChange(dataName, data, exceptEleId) {
	...
	for (let eleId in TIE_TREE[dataName]) {
		...
		for (let stateName in TIE_TREE[dataName][eleId]) {
			...
			if ("function" === typeof UPDATE_VIEWS[eleId]) {
				if (nullOrUndefined(binder.identifier) || chain.is(data).contain(binder.identifier) || isEmptyObject(binder.identifier)) {
...
function dispatchStateChange(state, eleId) {
	...
	for (let i = 0; i < binders.length; i++) {
		...
		if (binder.stateName in state) {
			dispatchDataChange(binder.dataName, state[binder.stateName], eleId);
		}
	}
}
/**
