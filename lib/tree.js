	...
	for (let eleId in TIE_TREE[dataName]) {
		...
		for (let stateName in TIE_TREE[dataName][eleId]) {
			...
			if ("function" === typeof UPDATE_VIEWS[eleId]) {
				if (nullOrUndefined(binder.identifier) || chain.is(binder.identifier).contain(identifier) || chain.is(data).contain(binder.identifier) || isEmptyObject(binder.identifier)) {
...
function dispatchStateChange(state, eleId) {
	...
	for (let i = 0; i < binders.length; i++) {
		...
		if (binder.stateName in state) {
			...
			dispatchDataChange(binder.dataName, binder.identifier,state[binder.stateName], eleId);
		}
	}
}
/**
 ...
 * @api public
 * **/
function getLastData(dataName, eleId, stateName) {
	_checkAndInitTieTree(dataName, eleId);
	if (TIE_TREE[dataName][eleId][stateName]) {
		return TIE_TREE[dataName][eleId][stateName].lastData;
	} else {
		return null;
	}
}
 ...
 * 
*/
function setLastData(dataName, eleId, stateName, data) {
	_checkAndInitTieTree(dataName, eleId);
	if (TIE_TREE[dataName][eleId][stateName]) {
		TIE_TREE[dataName][eleId][stateName].lastData = data;
	}
}
