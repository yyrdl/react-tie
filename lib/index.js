...
function _bind(reactEle, context) {
	...
	if (!reactEle._IS_TIED_) {
		...
		reactEle._originalUpdate = setState;
		...
		let updateView = function (state,binder) {
			if (!reactEle.is_unmount) {
				...
				if("function" == typeof binder.map && binder.stateName in state){
					  let temp = binder.map(state[binder.stateName]);
				...
				reactEle._originalUpdate(state);
			} else {
			...
			return {
				persist: function () {
					...
					for (let i = 0; i < stateNames.length; i++) {
						...
						if (stateName in state) {
							use(data_Name)._originalUpdate(stateNames[i].identifier, state[stateName], lastDatas[k]);
							k++;
						}
					}
...
Setting.prototype.mapTo = function (func) {
	this._package.map = func;
	return this;
};
...
function _hookUpdate(dataName, update) {
	return function (identifier, data, lastData) {
		Tree.dispatchDataChange(dataName, data, null);
		update(identifier, data, lastData);
	}
}
/**
 * @param {String} dataName
